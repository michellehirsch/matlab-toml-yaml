classdef jsontest < matlab.unittest.TestCase
    ...
    methods (Access = private)
        function verifyDataEqual(testCase, original, restored, context)
            ...
            % Handle JSONNull
            if isa(original, 'matlab.io.config.JSONNull')
                testCase.verifyTrue(isa(restored, 'matlab.io.config.JSONNull'), ...
                    sprintf('Expected JSONNull for %s', context));
                return;
            end
            ...
            for i = 1:length(origKeys)
                ...
                if isa(origVal, 'matlab.io.config.JSONNull')
                    % Null values
                    testCase.verifyTrue(isa(restVal, 'matlab.io.config.JSONNull'), ...
                        sprintf('Expected JSONNull for %s', keyContext));
                ...
                elseif iscell(origVal)
                    ...
                    for j = 1:numel(origVal)
                        ...
                        elseif isa(origVal{j}, 'matlab.io.config.JSONNull')
                            testCase.verifyTrue(isa(restVal{j}, 'matlab.io.config.JSONNull'), ...
                                sprintf('Expected JSONNull for %s{%d}', keyContext, j));
                        else
